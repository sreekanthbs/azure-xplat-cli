  ...
  describe('network', function() {
    ...
    describe('express-route authorization', function() {
      ...
      it('create should pass', function(done) {
        networkUtil.createGroup(groupName, location, suite, function() {
          networkUtil.createExpressRoute(groupName, expressRCPrefix, location, serviceProvider, peeringLocation, skuTier, skuFamily, tags1, suite, function() {
            var cmd = util.format('network express-route authorization create %s %s %s -k %s -u %s --json', groupName, expressRCPrefix, expressRCAuthPrefix, key1, status1).split(' ');
      ...
      it('set should modify express-route authorization', function(done) {
        var cmd = util.format('network express-route authorization set %s %s %s -k %s -u %s --json', groupName, expressRCPrefix, expressRCAuthPrefix, key2, status2).split(' ');
        testUtils.executeCommand(suite, retry, cmd, function(result) {
          result.exitStatus.should.equal(0);
          done();
        });
