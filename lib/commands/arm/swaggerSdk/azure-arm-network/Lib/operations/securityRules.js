  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse) {
          result = new client._models['SecurityRule'](parsedResponse);
        }
 ...
 * for this property include: 'Tcp', 'Udp', '*'.
 * 
 ...
 * â€œ*â€ can also be used to match all ports.
 * 
 ...
 * source address prefix. CIDR or source IP range. Asterix â€œ*â€ can also
 * be used to match all source IPs. Default tags such as
 * â€˜VirtualNetworkâ€™, â€˜AzureLoadBalancerâ€™ and â€˜Internetâ€™ can also
 ...
 * sets destination address prefix. CIDR or source IP range. Asterix â€œ*â€
 ...
 * traffic is allowed or denied. Possible values are â€œAllowâ€ and
 * â€œDenyâ€. Possible values for this property include: 'Allow', 'Deny'.
 * 
 ...
 * will be evaluated on incoming or outcoming traffic. Possible values for
 * this property include: 'Inbound', 'Outbound'.
 * 
...
SecurityRules.prototype.beginCreateOrUpdate = function (resourceGroupName, networkSecurityGroupName, securityRuleName, securityRuleParameters, options, callback) {
  ...
  try {
    if (securityRuleParameters) {
      requestModel = new client._models['SecurityRule'](securityRuleParameters);
    }
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse) {
          result = new client._models['SecurityRuleListResult'](parsedResponse);
        }
        if (parsedResponse !== null && parsedResponse !== undefined) {
          result.deserialize(parsedResponse);
        }
