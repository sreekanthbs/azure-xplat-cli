 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
PublicIPAddresses.prototype.get = function (resourceGroupName, publicIpAddressName, expand, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse) {
          result = new client._models['PublicIPAddress'](parsedResponse);
        }
 ...
 * or sets PrivateIP allocation method (Static/Dynamic). Possible values for
...
PublicIPAddresses.prototype.beginCreateOrUpdate = function (resourceGroupName, publicIpAddressName, parameters, options, callback) {
  ...
  try {
    if (parameters) {
      requestModel = new client._models['PublicIPAddress'](parameters);
    }
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse) {
          result = new client._models['PublicIPAddressListResult'](parsedResponse);
        }
        if (parsedResponse !== null && parsedResponse !== undefined) {
          result.deserialize(parsedResponse);
        }
