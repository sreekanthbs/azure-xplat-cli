 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
Subnets.prototype.get = function (resourceGroupName, virtualNetworkName, subnetName, expand, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse) {
          result = new client._models['Subnet'](parsedResponse);
        }
...
Subnets.prototype.beginCreateOrUpdate = function (resourceGroupName, virtualNetworkName, subnetName, subnetParameters, options, callback) {
  ...
  try {
    if (subnetParameters) {
      requestModel = new client._models['Subnet'](subnetParameters);
    }
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse) {
          result = new client._models['SubnetListResult'](parsedResponse);
        }
        if (parsedResponse !== null && parsedResponse !== undefined) {
          result.deserialize(parsedResponse);
        }
