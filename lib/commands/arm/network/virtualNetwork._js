...
__.extend(VirtualNetwork.prototype, {
  create: function (resourceGroupName, name, location, options, _) {
    ...
    try {
      self.networkManagementClient.virtualNetworks.createOrUpdate(resourceGroupName, name, vnet, _);
    } finally {
      progress.end();
    }
  ...
  list: function (resourceGroupName, _) {
    ...
    var vnets = null;
    try {
      vnets = self.networkManagementClient.virtualNetworks.list(resourceGroupName, _);
    } finally {
      progress.end();
    }
  ...
  delete: function (resourceGroupName, name, options, _) {
    ...
    try {
      self.networkManagementClient.virtualNetworks.deleteMethod(resourceGroupName, name, _);
    } finally {
      progress.end();
    }
  },
  ...
  get: function (resourceGroupName, name, message, _) {
    ...
    var vnet = null;
    try {
      vnet = self.networkManagementClient.virtualNetworks.get(resourceGroupName, name, _);
    } catch (e) {
      if (e.statusCode === 404) {
        return null;
      }
