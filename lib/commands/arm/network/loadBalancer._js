...
__.extend(LoadBalancer.prototype, {
  create: function (resourceGroupName, lbName, location, options, _) {
    ...
    try {
      self.networkManagementClient.loadBalancers.createOrUpdate(resourceGroupName, lbName, lbProfile, _);
    } finally {
      progress.end();
    }
  ...
  list: function (resourceGroupName, _) {
    ...
    var lbs = null;
    try {
      lbs = self.networkManagementClient.loadBalancers.list(resourceGroupName, _);
    } finally {
      progress.end();
    }
  ...
  update: function (resourceGroupName, lbName, parameters, _) {
    ...
    try {
      self.networkManagementClient.loadBalancers.createOrUpdate(resourceGroupName, lbName, parameters, _);
    } finally {
      progress.end();
    }
  },
