...
__.extend(LoadBalancer.prototype, {
  ...
  /**
   * Commands to manage outbound NAT Rules
   */

  createOutboundNatRule: function (resourceGroupName, lbName, ruleName, options, _) {
    var self = this;
    var lb = self.get(resourceGroupName, lbName, _);
    if (!lb) {
      throw new Error(util.format($('A load balancer with name "%s" not found in the resource group "%s"'), lbName, resourceGroupName));
    ...
    outboundRule = self._parseOutboundNatRule(resourceGroupName, lb, outboundRule, options, true);

    if (utils.findFirstCaseIgnore(lb.outboundNatRules, {name: ruleName})) {
      throw new Error(util.format($('An outbound NAT rule with name "%s" already exists in the load balancer "%s"'), ruleName, lbName));
    }

    lb.outboundNatRules.push(outboundRule);
    self.update(resourceGroupName, lbName, lb, _);

    var updatedLb = self.get(resourceGroupName, lbName, _);
    var updatedRule = utils.findFirstCaseIgnore(updatedLb.outboundNatRules, {name: ruleName});
    self.showOutboundNatRule(updatedRule);
  },

  setOutboundNatRule: function (resourceGroupName, lbName, ruleName, options, _) {
    var self = this;
    var lb = self.get(resourceGroupName, lbName, _);
    if (!lb) {
      throw new Error(util.format($('A load balancer with name "%s" not found in the resource group "%s"'), lbName, resourceGroupName));
    ...
    var outboundRule = utils.findFirstCaseIgnore(lb.outboundNatRules, {name: ruleName});
    if (!outboundRule) {
      throw new Error(util.format($('An outbound NAT rule with name "%s" not found in the load balancer "%s"'), ruleName, lbName));
    }

    self._parseOutboundNatRule(resourceGroupName, lb, outboundRule, options, false);
    self.update(resourceGroupName, lbName, lb, _);
  ...
  listOutboundNatRules: function (resourceGroupName, lbName, options, _) {
    var self = this;
    var lb = self.get(resourceGroupName, lbName, _);
    ...
    self.interaction.formatOutput(lb.outboundNatRules, function (outputData) {
      if (outputData.length === 0) {
        self.output.warn($('No outbound NAT rules found'));
      } else {
  ...
  showOutboundNatRule: function (rule) {
    var self = this;
    self.interaction.formatOutput(rule, function (rule) {
  ...
  deleteOutboundNatRule: function (resourceGroupName, lbName, ruleName, options, _) {
    var self = this;
    var lb = self.get(resourceGroupName, lbName, _);
    ...
    var ruleIndex = utils.indexOfCaseIgnore(lb.outboundNatRules, {name: ruleName});
    if (ruleIndex === -1) {
      throw new Error(util.format($('An outbound NAT rule with name "%s" not found in the load balancer "%s"'), ruleName, lbName));
    }

    if (!options.quiet && !self.interaction.confirm(util.format($('Delete outbound NAT rule %s? [y/n] '), ruleName), _)) {
      return;
    }

    lb.outboundNatRules.splice(ruleIndex, 1);
    self.update(resourceGroupName, lbName, lb, _);
  },
  ...
  _parseOutboundNatRule: function (resourceGroupName, lb, outboundRule, options, useDefaults) {
//    var self = this;

    outboundRule.allocatedOutboundPorts = options.allocatedOutboundPorts;
    outboundRule.frontendIPConfigurations = [{
      id: '/subscriptions/5893435f-6990-4bb3-bebb-b12f3535f990/resourceGroups/test-roman-group/providers/Microsoft.Network/loadBalancers/MyEmpty/frontendIPConfigurations/MyFrontendIP'
    }];
    outboundRule.backendAddressPool = {
      id: '/subscriptions/5893435f-6990-4bb3-bebb-b12f3535f990/resourceGroups/test-roman-group/providers/Microsoft.Network/loadBalancers/MyEmpty/backendAddressPools/MyAddressPool'
    };

    /*if (options.allocatedOutboundPorts) {
     */
    /*var protocolValidation = self.endpointUtil.validateProtocol(options.protocol, 'protocol');
     if (protocolValidation.error) {
     throw new Error(protocolValidation.error);
    ...
    /*options.protocol = constants.lb.defProtocol;
     self.output.warn(util.format($('Using default protocol: %s'), options.protocol));
     outboundRule.protocol = options.protocol;*/
    /*
     }

     if (options.frontendIp) {
     var fipNames = options.frontendIp.split(',');
     outboundRule.frontendIPConfigurations = [];

     for (var num in fipNames) {
     var fipName = fipNames[num];
     var frontendIpConfFound = utils.findFirstCaseIgnore(lb.frontendIpConfigurations, {name: fipName});
     if (!frontendIpConfFound) {
     throw new Error(util.format($('Frontend IP config "%s" not found'), fipName));
     }
     outboundRule.frontendIPConfigurations.push({
     id: frontendIpConfFound.id
     });
     }
     } else if (useDefaults) {
     if (!outboundRule.frontendIPConfiguration) {
     if (lb.frontendIpConfigurations.length === 0) {
     throw new Error(util.format($('Load balancer with name "%s" has no frontend IP configurations'), lb.name));
     }
     outboundRule.frontendIPConfiguration = {
     id: lb.frontendIpConfigurations[0].id
     };
     self.output.warn($('Setting default inbound rule frontend IP configuration'));
     }
