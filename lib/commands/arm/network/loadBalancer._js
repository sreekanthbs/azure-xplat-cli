    ...
    self.interaction.formatOutput(pool, function (pool) {
      ...
      if (!__.isEmpty(pool.backendIpConfigurations)) {
        ...
        pool.backendIpConfigurations.forEach(function (backendIpConfig) {
          self.output.listItem(backendIpConfig.id);
        });
  ...
  _showBalancingRule: function (rule) {
    ...
    self.interaction.formatOutput(rule, function (rule) {
      ...
      if (rule.frontendIPConfiguration) {
        self.output.nameValue($('Frontend IP configuration'), rule.frontendIPConfiguration.id);
      }
      if (rule.backendAddressPool) {
        self.output.nameValue($('Backend address pool'), rule.backendAddressPool.id);
      }
      if (rule.probe) {
        self.output.nameValue($('Probe'), rule.probe.id);
      }
    });
  },
  ...
  _showInboundNatRule: function (rule) {
    ...
    self.interaction.formatOutput(rule, function (rule) {
      ...
      if (rule.backendIPConfiguration) {
        self.output.nameValue($('Backend IP Configuration:  '), rule.backendIPConfiguration.id);
      }
    });
  },
