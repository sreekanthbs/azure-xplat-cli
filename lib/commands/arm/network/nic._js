...
__.extend(Nic.prototype, {
  ...
  update: function (resourceGroupName, nicName, nicProfile, _) {
    ...
    var progress = self.interaction.progress(util.format($('Updating network interface "%s"'), nicName));
    try {
      self.networkManagementClient.networkInterfaces.createOrUpdate(resourceGroupName, nicName, nicProfile, _);
    } finally {
      progress.end();
    }
  },
  ...
  _updateAddressPool: function (resourceGroupName, nicName, options, isAdding, _) {
    ...
    if (options.lbAddressPoolId) {
      ...
      var addressPool = utils.findFirstCaseIgnore(lb.backendAddressPools, {name: options.addressPoolName});
  ...
  _updateInboundRule: function (resourceGroupName, nicName, options, isAdding, _) {
    ...
    if (options.inboundNatRuleId) {
      ...
      var inboundNatRule = utils.findFirstCaseIgnore(lb.inboundNatRules, {name: options.inboundNatRuleName});
      if (!inboundNatRule) {
  ...
  _parseNic: function (resourceGroupName, nic, options, _) {
    ...
    if (options.enableIpForwarding) {
      if (!utils.ignoreCaseEquals(options.enableIpForwarding, 'true') && !utils.ignoreCaseEquals(options.enableIpForwarding, 'false')) {
        throw new Error($('--enable-ip-forwarding parameter must be true or false'));
      }
      nic.enableIPForwarding = options.enableIpForwarding;
    }
    ...
    if (options.lbAddressPoolIds) {
      if (utils.argHasValue(options.lbAddressPoolIds)) {
        ...
        poolIds.forEach(function (poolId) {
          poolId = poolId.replace(/'|''$/gm, ''); // TODO what?
          var pool = {
            id: poolId
          };
    ...
    if (options.lbInboundNatRuleIds) {
      if (utils.argHasValue(options.lbInboundNatRuleIds)) {
        ...
        natIds.forEach(function (natId) {
          natId = natId.replace(/'|''$/gm, ''); // TODO what?
          var nat = {
            id: natId
          };
