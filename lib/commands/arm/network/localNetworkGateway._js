  ...
  list: function (options, _) {
    ...
    try {
      if (options.resourceGroup) {
        gateways = self.networkManagementClient.localNetworkGateways.list(options.resourceGroup, _);
      } else {
        // TODO no listAll method
        gateways = self.networkManagementClient.localNetworkGateways.listAll(_);
      }
    } finally {
      progress.end();
    }
