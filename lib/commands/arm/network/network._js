...
exports.init = function (cli) {
  ...
  var lbOutboundNatRule = lb.category('outbound-nat-rule')
    .description($('Commands to manage load balancer outbound NAT rules'));

  lbOutboundNatRule.command('create [resource-group] [lb-name] [name]')
    .description($('Add a load balancing outbound NAT rule to the load balancer'))
    .usage('[options] <resource-group> <lb-name> <name>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-l, --lb-name <lb-name>', $('the name of the load balancer'))
    .option('-n, --name <name>', $('the name of the outbound NAT rule'))
    .option('-b, --backend-address-pool <backend-address-pool>', $('name of the backend address pool'))
    .option('-f, --frontend-ip <frontend-ip>', $('the name of the frontend ip configuration'))
    .option('-p, --allocated-outbound-ports <frontend-ip>', $('the allocated outbound ports'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, lbName, name, options, _) {
      resourceGroup = cli.interaction.promptIfNotGiven($('Resource group name: '), resourceGroup, _);
      lbName = cli.interaction.promptIfNotGiven($('Load balancer name: '), lbName, _);
      name = cli.interaction.promptIfNotGiven($('Outbound rule name: '), name, _);

      var networkResourceProviderClient = getNetworkResourceProviderClient(options);
      var loadBalancer = new LoadBalancer(cli, networkResourceProviderClient);
      loadBalancer.createOutboundNatRule(resourceGroup, lbName, name, options, _);
    });

  lbOutboundNatRule.command('set [resource-group] [lb-name] [name]')
    .usage('[options] <resource-group> <lb-name> <name>')
    .description($('Set a load balancing outbound NAT rule of load balancer'))
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-l, --lb-name <lb-name>', $('the name of the load balancer'))
    .option('-n, --name <name>', $('the name of the outbound NAT rule'))
    .option('-b, --backend-address-pool <backend-address-pool>', $('name of the backend address pool'))
    ...
    .execute(function (resourceGroup, lbName, name, options, _) {
      ...
      var networkResourceProviderClient = getNetworkResourceProviderClient(options);
      var loadBalancer = new LoadBalancer(cli, networkResourceProviderClient);
      loadBalancer.setOutboundNatRule(resourceGroup, lbName, name, options, _);
    });

  lbOutboundNatRule.command('list [resource-group] [lb-name]')
    .usage('[options] <resource-group> <lb-name>')
    .description($('Get all load balancing outbound NAT rules of load balancer'))
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-l, --lb-name <lb-name>', $('the name of the load balancer'))
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, lbName, options, _) {
      ...
      var networkResourceProviderClient = getNetworkResourceProviderClient(options);
      var loadBalancer = new LoadBalancer(cli, networkResourceProviderClient);
      loadBalancer.listOutboundNatRules(resourceGroup, lbName, options, _);
    });

  lbOutboundNatRule.command('delete [resource-group] [lb-name] [name]')
    .usage('[options] <resource-group> <lb-name> <name>')
    .description($('Delete a load balancing outbound NAT rule from a load balancer'))
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-l, --lb-name <lb-name>', $('the name of the load balancer'))
    .option('-n, --name <name>', $('the name of the outbound NAT rule'))
    .option('-q, --quiet', $('quiet mode, do not ask for delete confirmation'))
    ...
    .execute(function (resourceGroup, lbName, name, options, _) {
      ...
      var networkResourceProviderClient = getNetworkResourceProviderClient(options);
      var loadBalancer = new LoadBalancer(cli, networkResourceProviderClient);
      loadBalancer.deleteOutboundNatRule(resourceGroup, lbName, name, options, _);
    });
