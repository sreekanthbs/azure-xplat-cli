...
__.extend(RouteTable.prototype, {
  create: function (resourceGroupName, routeTableName, location, options, _) {
    ...
    try {
      self.networkManagementClient.routeTables.createOrUpdate(resourceGroupName, routeTableName, parameters, _);
    } finally {
      progress.end();
    }
  ...
  list: function (resourceGroupName, options, _) {
    ...
    var routeTables;
    try {
      routeTables = self.networkManagementClient.routeTables.list(resourceGroupName, _);
    } finally {
      progress.end();
    }
  ...
  get: function (resourceGroupName, routeTableName, _) {
    ...
    var routeTable;
    try {
      routeTable = self.networkManagementClient.routeTables.get(resourceGroupName, routeTableName, _);
    } catch (e) {
      if (e.statusCode === 404) {
        return null;
      } else {
