...
__.extend(ExpressRoute.prototype, {
  ...
  showPeering: function (resourceGroupName, circuitName, name, options, _) {
    ...
    self.interaction.formatOutput(peering, function (peering) {
      if (peering === null) {
        ...
        self.output.nameValue($('vlanId'), peering.vlanId, 2);
        ...
        self.output.nameValue($('Advertised Public Prefix State'), peering.microsoftPeeringConfig.advertisedPublicPrefixState, 4);
  ...
  listPeering: function (resourceGroupName, circuitName, options, _) {
    ...
    self.interaction.formatOutput(peerings.peerings, function (peerings) {
      if (peerings.length === 0) {
        ...
        self.output.table(peerings.peerings, function (row, peering) {
  ...
  getPeering: function (resourceGroupName, circuitName, name, _) {
    var self = this;
    var progress = self.interaction.progress(util.format($('Looking up the express route circuit peering "%s"'), circuitName));
  ...
  _parseCircuitPeering: function (peering, options, useDefaults) {
    peering.properties = {};
    if (options.type) {
      utils.verifyParamExistsInCollection(constants.expressRoute.peering.type, options.type, '--type');
      peering.peeringType = options.type;
    } else {
      if (useDefaults) {
    ...
    if (options.state) {
      utils.verifyParamExistsInCollection(constants.expressRoute.peering.state, options.state, '--state');
      peering.state = options.state;
    } else {
      if (useDefaults) {
        peering.state = constants.expressRoute.peering.state[0];
      }
    }
    ...
    if(options.msAdvertisedPublicPrefixes) {
      peering.microsoftPeeringConfig.advertisedpublicprefixes = options.msAdvertisedPublicPrefixes.split(',');
    }

    if (options.msAdvertisedPublicPrefixState) {
      utils.verifyParamExistsInCollection(constants.expressRoute.peering.publicPrefixState, options.msAdvertisedPublicPrefixState, '--ms-advertised-public-prefix-state');
      peering.microsoftPeeringConfig.advertisedPublicPrefixState = options.msAdvertisedPublicPrefixState;
    } else {
      if (useDefaults) {
        peering.microsoftPeeringConfig.advertisedPublicPrefixState = constants.expressRoute.peering.publicPrefixState[0];
      }
    }
    ...
    if (options.msRoutingRegistryName) {
      utils.verifyParamExistsInCollection(constants.expressRoute.peering.registryName, options.msRoutingRegistryName, '--ms-routing-registry-name');
      peering.microsoftPeeringConfig.advertisedPublicPrefixState = options.msRoutingRegistryName;
    } else {
      if (useDefaults) {
        peering.microsoftPeeringConfig.advertisedPublicPrefixState = constants.expressRoute.peering.registryName[0];
      }
    }
