  ...
  _parseCircuitPeering: function (peering, options, useDefaults) {
    ...
    if (options.state) {
      ...
      if (useDefaults) {
        ...
        self.output.warn(util.format($('Using default peering state: %s'), peering.state));
      }
    }
    ...
    if (options.msAdvertisedPublicPrefixState) {
      peering.microsoftPeeringConfig.advertisedPublicPrefixState = utils.verifyParamExistsInCollection(
