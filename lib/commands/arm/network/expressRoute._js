/**
 * Created by Mikhail on 12-Oct-15.
 */
...
__.extend(ExpressRoute.prototype, {
    ...
    createAuth: function (resourceGroupName, circuitName, name, options, _) {
      var self = this;
      ...
      var circuitAuth = self.getAuth(resourceGroupName, circuitName, name, _);
      if (circuitAuth) {
      ...
      circuitAuth = self._parseCircuitAuth(circuitAuth, options, true);
      ...
      self.showAuth(resourceGroupName, circuitName, name, options, _);
    },

    setAuth: function (resourceGroupName, circuitName, name, options, _) {
      var self = this;
      var circuit = self.get(resourceGroupName, circuitName, _);
      if (!circuit) {
        throw new Error(util.format($('An express route circuit with name "%s" not found in the resource group "%s"'), circuitName, resourceGroupName));
      ...
      circuitAuth = self._parseCircuitAuth(circuitAuth, options, false);
    ...
    showAuth: function (resourceGroupName, circuitName, name, options, _) {
      var self = this;
      var circuitAuth = self.getAuth(resourceGroupName, circuitName, name, _);
      if (!circuitAuth) {
    ...
    deleteAuth: function (resourceGroupName, circuitName, name, options, _) {
      var self = this;
    ...
    listAuth: function (resourceGroupName, circuitName, options, _) {
      var self = this;
    ...
    getAuth: function (resourceGroupName, circuitName, name, _) {
      var self = this;
    ...
    _parseCircuitAuth: function (circuitAuth, options, useDefaults) {
      if (options.key) {
        circuitAuth.authorizationKey = options.key.toString('base64');
      }
      if (options.status) {
        var status = utils.verifyParamExistsInCollection(constants.expressRouteAuth.status, options.status, 'authorization use status');
        if (!status) {
          throw new Error(util.format($('The express route circuit authorization status, valid values are [%s}'), constants.expressRouteAuth.status));
        }
        circuitAuth.authorizationUseStatus = options.status;
      } else {
        if (useDefaults) {
          circuitAuth.authorizationUseStatus = constants.expressRouteAuth.status[0];
        }
      }
      return circuitAuth;
    }
