...
__.extend(ExpressRoute.prototype, {
  ...
  _parseCircuitAuthorization: function (circuitAuth, options, useDefaults) {
    ...
    if (options.status) {
      circuitAuth.authorizationUseStatus = utils.verifyParamExistsInCollection(constants.expressRouteAuthorization.status, options.status, '--status');
    } else {
      if (useDefaults) {
        circuitAuth.authorizationUseStatus = constants.expressRouteAuthorization.status[0];
        self.output.warn(util.format($('Using default authorization use status: %s'), constants.expressRouteAuthorization.status[0]));
      }
    }
    return circuitAuth;
  }
