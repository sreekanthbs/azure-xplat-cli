...
__.extend(VirtualNetworkGateway.prototype, {
  ...
  createConnection: function (resourceGroupName, connectionName, options, _) {
    ...
    try {
      self.networkManagementClient.virtualNetworkGatewayConnections.createOrUpdate(resourceGroupName, connectionName, parameters, _);
    } finally {
      progress.end();
    }
  ...
  setConnection: function (resourceGroupName, connectionName, options, _) {
    ...
    var resInfo1 = resourceUtils.getResourceInformation(connection.virtualNetworkGateway1.id);
    connection.virtualNetworkGateway1 = self.get(resourceGroupName, resInfo1.resourceName, _);

    if (connection.virtualNetworkGateway2) {
      var resInfo2 = resourceUtils.getResourceInformation(connection.virtualNetworkGateway2.id);
      connection.virtualNetworkGateway2 = self.get(resourceGroupName, resInfo2.resourceName, _);
    } else {
      var resInfo3 = resourceUtils.getResourceInformation(connection.localNetworkGateway2.id);
      connection.localNetworkGateway2 = self.lnetGatewayCrud.get(resourceGroupName, resInfo3.resourceName, _);
    }
    console.log('%j', connection);

    connection = self._parseConnection(resourceGroupName, connection, options, false, _);

    var progress = self.interaction.progress(util.format($('Updating virtual network gateway connection "%s"'), connectionName));
    try {
      self.networkManagementClient.virtualNetworkGatewayConnections.createOrUpdate(resourceGroupName, connectionName, connection, _);
    } finally {
      progress.end();
    }
    self.showConnection(resourceGroupName, connectionName, options, _);
  ...
  _createOrUpdate: function (resourceGroupName, gatewayName, parameters, isCreating, _) {
    ...
    try {
      self.networkManagementClient.virtualNetworkGateways.createOrUpdate(resourceGroupName, gatewayName, parameters, _);
    } finally {
      progress.end();
    }
  },
